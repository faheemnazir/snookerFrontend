...
const courses = [
  {
    ...
    desc: "An immersive residential program focused on high-pressure frame management and break building techniques used by top-16 professionals.",
    rating: "4.9",
    duration: "2 Weeks",
    ...
    <div className="bg-background min-h-screen text-white relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <section className="py-24 px-6 text-center">
          ...
          <p className="text-muted-foreground font-light text-sm max-w-2xl mx-auto leading-relaxed">
...
<section className="px-6 pb-24">
  <div className="max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    {coursesLoading ? (
      ...
      (coursesList.length > 0 ? coursesList : courses).map(
        (course, i) => {
          return (
            <div
              key={i}
              className="group relative overflow-hidden rounded-2xl border border-white/10 bg-card/60 backdrop-blur-xl hover:border-accent/40 hover:-translate-y-2 transition-all duration-500 flex flex-col"
            >
              {/* GLOW */}
              ...
              <div className="aspect-[16/10] overflow-hidden relative">
                ...
                <div className="absolute top-4 left-4">
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] px-3 py-2 rounded-full border backdrop-blur-md bg-accent/10 text-accent border-accent/30">
                    {course.duration}
                  </span>
                </div>
                ...
                <div className="absolute top-4 right-4">
                  <div className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest bg-black/50 backdrop-blur-md border border-white/10 px-3 py-2 rounded-full text-accent">
                    <span>★</span>
              ...
              <div className="p-7 flex flex-col flex-1">
                {/* TITLE */}
                <div className="mb-5">
                  <h2 className="font-heading text-2xl font-bold text-white mb-2 group-hover:text-accent transition-colors">
                    {course.title}
                  </h2>

                  <p className="text-[10px] uppercase tracking-[0.25em] text-accent font-bold mb-4">
                    {course.subtitle}
                  </p>
                ...
                <div className="space-y-3 mb-7">
                  {(course.details || [])
                    ...
                    .map((item, idx) => (
                      <div
                        key={idx}
                        className="rounded-xl border border-white/10 bg-white/[0.03] p-4 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.05]"
                      >
                ...
                <button
                  ...
                  className="w-full text-xs font-bold uppercase tracking-widest py-3 bg-transparent border border-accent text-accent hover:bg-accent hover:text-background transition-all duration-300 rounded-lg"
                >
                  Enroll Now
                </button>
              </div>
